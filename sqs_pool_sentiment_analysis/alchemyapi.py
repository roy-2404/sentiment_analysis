...
class AlchemyAPI:
    ...
    def __init__(self, api_key):
        ...
        Argument provides the api key. If its not 40 char, we exit with an error message
        """
        import sys
        if api_key == '' or len(api_key) != 40:
            print('The api key provided is invalid. Exiting...')
            sys.exit(0)
        else:
            self.apikey = api_key
